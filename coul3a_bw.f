     ...
     +     ,ang12,ang13,ExBe8inf         

      PARAMETER(Nmont=200, Nbig=1E7)
      REAL Xbw(Nbig),Ybw(Nmont),Xangle(Nbig),Yangle(Nmont)
      INTEGER Nevent
      EXTERNAL bwfunc, anglefunc
      ...
      open(31,file='output/bw_r10fm_N5000_ad.dat',
     +      status='unknown')
      

*     Sample excitation energies
      call funlxp(bwfunc, Ybw, 1.5, 6.0)                
      call funlux(Ybw, Xbw, Nevent)

*     Sample excitation energies
      call funlxp(anglefunc, Yangle, 0., 3.1415)                
      call funlux(Yangle, Xangle, Nevent)
...
*      Be8 Excitation energy
         ExBe8inf = 0.5*Malpha*(Vx_a2**2.+Vy_a2**2.
     +   +Vx_a3**2.+Vy_a3**2.) - 0.092


*     Write output
         write(31,*) real(ExBe8),',',real(ExBe8inf)
      ...
      real function bwfunc(x)
      real G0, E0
      E0 = 3.0
      G0 = 1.5
      bwfunc = (G0/2.)**2. / ((x - E0)**2. + (G0/2.)**2.)
      end


      real function anglefunc(x)
*      anglefunc = sin(x)
      anglefunc = sin(x) * (1. - sin(x)**2.)
      end
